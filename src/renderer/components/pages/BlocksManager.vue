    ...
    <Transition>
        <div class="flex flex-col">
            <div class="flex flex-row py-1 px-2" id="buttons" ref="buttons">
                <button v-for="btn in actionButtons" id="btn.id" class="btn btn-primary" @click="btn.action" type="button"
                    v-html="btn.title"></button>
            </div>
            <div id="main" class="flex flex-row">
                <div id="board" ref="board" class="relative">
                    ...
                    <div v-for="(block, index) in allBlocks" class="block" @mousedown.prevent="ev => onStartMove(ev, block)"
                        @mousemove.prevent="ev => onMove(ev, block)" :style="block.style" :id="`block${index}`"
                        v-html="index + 1"></div>
                </div>
                <div id="right">
                    <div id="msg">
            ...
            <Modal id="save-as" ref="saveAs">
                <div class="input-group mb-3">
                    ...
                    <input type="text" class="form-control" placeholder="Username" aria-label="Save Name"
                        aria-describedby="basic-addon1" v-model="input.name">
...
const actionButtons = computed(() => {
    return [
        { id: 'loadGame', title: 'Load Game', action: getGames },
